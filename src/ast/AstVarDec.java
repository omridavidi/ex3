...
public class AstVarDec extends AstDec
{
    ...
    public Type semantMe(){
		...
		if (SymbolTable.isReservedKeyword(name)) throw new SemanticException(this.getLine(), "variable '" + name + "' cannot use reserved keyword");
		
		Type t = type.semantMe();
		if (t == null || t == TypeVoid.getInstance())
            throw new SemanticException(this.getLine(), "variable '" + name + "' cannot be of type void or null");
		
		if (SymbolTable.getInstance().lookupLocal(name) != null)
            throw new SemanticException(this.getLine(), "variable '" + name + "' already declared in current scope");

		if (exp != null){
			Type expType = exp.semantMe();
			if (expType == TypeNil.getInstance()){
				if(!(t instanceof TypeClass) && !(t instanceof TypeArray))
                    throw new SemanticException(this.getLine(), "cannot assign nil to non-class/array type");
			}
			...
			if (t instanceof TypeArray && expType instanceof TypeArray && exp instanceof AstNewExpArr)
    {
				...
				if (arrayType.arrayDataType != newArrayType.arrayDataType) {
					throw new SemanticException(this.getLine(), "variable initialization type mismatch for '" + name + "'");
				}
