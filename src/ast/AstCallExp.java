...
public class AstCallExp extends AstExp
{
	...
	public Type semantMe()
	{
		...
		if (var == null)
		{
			...
			if (!(varFunc instanceof TypeFunction)) throw new SemanticException(this.getLine(), "'" + name + "' is not a function");
			...
			if (!(object instanceof TypeClass)) throw new SemanticException(this.getLine(), "cannot call method on non-class type");
		...
		while(expectedParameters != null && providedParameters != null)
		{
			if (!expectedParameters.head.isCompatibleWith(providedParameters.head))
				{
					throw new SemanticException(this.getLine(), "parameter type mismatch in call to '" + name + "'");
				}
		...
		if (expectedParameters != null || providedParameters != null) throw new SemanticException(this.getLine(), "parameter count mismatch in call to '" + name + "'");
