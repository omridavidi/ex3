	...
	public Type semantMe()
	{
		...
		switch(this.op){
			case OP_PLUS:
				...
				throw new SemanticException(this.getLine(), "'+' operator requires both operands to be int or both to be string");

            case OP_MINUS:
				// only integers
				if (t1 == TypeInt.getInstance() && t2 == TypeInt.getInstance())
					return TypeInt.getInstance();
				throw new SemanticException(this.getLine(), "'-' operator requires both operands to be int");
			...
			case OP_DIVIDE:
                ...
                if (right instanceof AstExpInt) {
                    ...
                    if (rightInt.value == 0)
						throw new SemanticException(this.getLine(), "division by zero");
                }
                return TypeInt.getInstance();
			...
			case OP_EQ:
				...
				throw new SemanticException(this.getLine(), "'=' operator requires compatible operand types");

			default:
				throw new SemanticException(this.getLine(), "unknown binary operator:" + this.op);
		}
	}
}
