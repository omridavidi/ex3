...
public class AstNewExpArr extends AstNewExp
{
    ...
    public Type semantMe(){
        ...
        if (elementType == null)
            throw new SemanticException(this.getLine(), "array element type not declared");

        // rule 1: NON VOID type.
        if (elementType == TypeVoid.getInstance())
            throw new SemanticException(this.getLine(), "array cannot be allocated over void type");
        
        // rule 2: The size expression e must be of type int.
        Type sizeType = size.semantMe();
        if (sizeType != TypeInt.getInstance())
            throw new SemanticException(this.getLine(), "array size must be of type int");

        // rule 3: If e is a constant expression, it must be greater than 0.
        if (size instanceof AstExpInt) {
            AstExpInt sizeInt = (AstExpInt) size;
            if (sizeInt.value <= 0) throw new SemanticException(this.getLine(), "array size must be greater than 0");
        }
