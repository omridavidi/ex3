...
public class AstArrayTypeDec extends AstDec
{
	...
	public TypeArray semantMe(){
		...
		if (SymbolTable.isReservedKeyword(name)) throw new SemanticException(this.getLine(), "array type '" + name + "' cannot use reserved keyword");

		// Array type declarations can only appear in global scope
		if (!SymbolTable.getInstance().getScope().equals("GLOBAL")) throw new SemanticException(this.getLine(), "array type '" + name + "' can only be declared in global scope");
		...
		if (elementType == TypeVoid.getInstance())
			{
				throw new SemanticException(this.getLine(), "array type '" + name + "' cannot be defined over void type");
			}
		
		// rule 2: An array type must be defined over a previously declared (non-void) type.
		if (SymbolTable.getInstance().lookupLocal(name) != null)
			{
				throw new SemanticException(this.getLine(), "array type '" + name + "' already declared in current scope");
			}
