...
public class AstVarSubscript extends AstExpVar
{
	...
	public Type semantMe()
	{
		...
		if (!(type instanceof TypeArray))
            throw new RuntimeException("semantic error: cannot subscript non-array type");
		
		// rule 2: The subscript expression e must be of type int.
        Type subscriptType = subscript.semantMe();
        if (subscriptType != TypeInt.getInstance())
            throw new RuntimeException("semantic error: array subscript must be of type int");
		...
		if (subscript instanceof AstExpInt)
		{
			...
			if (constExp.value < 0)
				throw new RuntimeException("semantic error: array subscript must be non-negative");
		}
